...
const schema = yup.object().shape({
  userLoggin: yup.string().required('El usuario o correo es obligatorio').trim(),
  password: yup.string()
...
const Login = () => {
  ...
  const onSubmit = async (data) => {
    try {
      const response = await axios.post('http://localhost:3616/login', {
  ...
  return (
    <div className="auth-container">
      <h1 className="auth-title">Gestión Almacenadora</h1>
